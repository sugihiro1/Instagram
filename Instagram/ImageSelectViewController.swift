...
class ImageSelectViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, AdobeUXImageEditorViewControllerDelegate {
  ...
  func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
    if info[UIImagePickerControllerOriginalImage] != nil {
      ...
      // AdobeUXImageEditorを起動する　-> AdobeUXImageEditorで、受け取ったimageを加工できる
  ...
  // AdobeImageEditorで加工が終わったときに呼ばれるメソッド  = AdobeImageEditor画面で「完了」ボタンをタップ
  ...
  // AdobeImageEditorで加工をキャンセルしたときに呼ばれる  = AdobeImageEditor画面で「キャンセル」ボタンをタップ
